...
(function (global) {
  ...
  function buildSection(id, meta, bodyNode) {
    var section = el('section', 'aos-section aos-requirements__section aos-requirements__section--' + id);
  ...
  function buildHealthStrip(items) {
    var strip = el('div', 'aos-requirements__health');
    strip.setAttribute('role', 'group');
    strip.setAttribute('aria-label', 'Requirement health');
    asArray(items).forEach(function (item) {
      var node = el('span', 'aos-requirements__health-item');
      node.setAttribute('aria-label', item.label + ': ' + item.status);
      var dot = el('span', 'aos-requirements__health-dot' + (item.tone ? ' aos-requirements__health-dot--' + item.tone : ''));
      dot.setAttribute('aria-hidden', 'true');
      node.appendChild(dot);
      node.appendChild(el('span', 'aos-requirements__health-label', item.label));
      node.appendChild(el('span', 'aos-requirements__health-status', item.status));
   ...
   * its click to swap the Requirement Inspector into the detail mount; selecting
   * the first row establishes the default detail. Memory-only presentation state —
   * no business data is touched, no route changed.
   */
  function createSelection(detailMount, context) {
    ...
    function select(index) {
      entries.forEach(function (entry, entryIndex) {
        ...
        entry.node.classList.toggle('aos-requirements__row--selected', selected);
        entry.node.setAttribute('aria-pressed', selected ? 'true' : 'false');
      });
      if (entries[index]) {
        detailMount.replaceChildren(presentation().inspectorPanel(buildRequirementInspector(entries[index].row.requirement, context)));
      }
    }
    return {
  ...
  function mountRailGroups(listNode, detailMount, groups, context) {
    ...
    asArray(groups).forEach(function (group) {
      if (group.label) {
        var divider = el('div', 'aos-requirements__group');
        divider.setAttribute('role', 'separator');
        divider.setAttribute('aria-label', group.label);
        divider.appendChild(el('span', 'aos-requirements__group-label', group.label));
        divider.appendChild(el('span', 'aos-requirements__group-count aos-numeric', String(asArray(group.rows).length)));
        listNode.appendChild(divider);
      }
      asArray(group.rows).forEach(function (row) {
        var node = buildRow(row);
        node.classList.add('aos-requirements__row');
  ...
  function buildLineageBody(lineage) {
    var chain = el('div', 'aos-requirements__lineage');
    chain.setAttribute('role', 'list');
    asArray(lineage).forEach(function (node, index) {
      if (index > 0) {
        var connector = el('span', 'aos-requirements__lineage-connector', '→');
        connector.setAttribute('aria-hidden', 'true');
        chain.appendChild(connector);
      }
      var tag = node.path ? 'a' : 'div';
      var card = el(tag, 'aos-requirements__lineage-node' + (node.highlighted ? ' aos-requirements__lineage-node--highlighted' : '') + (node.present ? '' : ' aos-requirements__lineage-node--empty'));
      card.setAttribute('role', 'listitem');
      if (node.path) {
        card.setAttribute('href', '#/' + node.path);
      }
      card.appendChild(el('span', 'aos-requirements__lineage-label', node.label));
      var value = node.count === null ? (node.present ? '' : '—') : String(node.count);
      if (value) {
        card.appendChild(el('span', 'aos-requirements__lineage-count aos-numeric', value));
      }
      if (node.hint) {
        card.appendChild(el('span', 'aos-requirements__lineage-hint', node.hint));
      }
  ...
  function buildFooterItems(entries) {
    ...
    asArray(entries).forEach(function (entry) {
      var item = el('span', 'aos-requirements-footer__item');
      item.appendChild(el('span', 'aos-requirements-footer__label', entry.label));
      item.appendChild(el('span', 'aos-requirements-footer__value aos-numeric', entry.value));
